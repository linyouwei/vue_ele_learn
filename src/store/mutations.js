...
export default {
	...
	[ADD_CART](state, {
		...
		let cart = state.cartList;//购物车列表
		let shop = cart[shopid] = (cart[shopid] || {});//shopid：商品所属的商店
		let category = shop[category_id] = (shop[category_id] || {});//category_id：商品所属的分类
		let item = category[item_id] = (category[item_id] || {});//通过商品的item_id，得到商品
		if (item[food_id]) {
			item[food_id]['num']++;
		} else {
			item[food_id] = {
